...
export default function SettingsScreen() {
  const { colors, setTheme, themePreference, isDark } = useAppTheme();
    ...
    <SafeAreaView style={styles.safeArea}>
      ...
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 120 }}>
        ...
        <TouchableOpacity 
          ...
          onPress={() => Linking.openURL('https://creatorohmyhindu.vercel.app/')}
        >
        ...
        <Modal transparent animationType="fade" visible={showHelpDialog}>
          <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowHelpDialog(false)}>
            <View style={styles.dialog}>
              <Text style={{ fontWeight: 'bold', fontSize: 18, color: colors.DarkText, marginBottom: 12 }}>Help Center</Text>
              ...
              <TouchableOpacity style={{ paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: colors.Slate100 }} onPress={() => Linking.openURL('mailto:support@ohmyhindustan.com')}>
      ...
      {showAboutDialog && (
        <Modal transparent animationType="fade" visible={showAboutDialog}>
          <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowAboutDialog(false)}>
            <View style={styles.dialog}>
              <Text style={{ fontWeight: 'bold', fontSize: 18, color: colors.DarkText, marginBottom: 8 }}>About Jan Samvad</Text>
      ...
      {showNotifSettings && (
        <Modal transparent animationType="fade" visible={showNotifSettings}>
          <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowNotifSettings(false)}>
            <View style={styles.dialog}>
              <Text style={{ fontWeight: 'bold', fontSize: 18, color: colors.DarkText, marginBottom: 16 }}>Notification Settings</Text>
...
const getStyles = (colors: any) => StyleSheet.create({
  safeArea: { flex: 1, backgroundColor: colors.Slate50, paddingTop: Platform.OS === 'android' ? 24 : 0 },
  header: { padding: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: colors.Slate50 },
  headerTitle: { fontSize: 22, fontWeight: 'bold', color: colors.DarkText },
  userHeader: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: colors.SurfaceWhite,
    padding: 16, borderRadius: 16, borderWidth: 1, borderColor: colors.Slate200, marginBottom: 24,
  },
  userAvatar: { width: 56, height: 56, borderRadius: 28, borderWidth: 2, borderColor: colors.PrimaryRed },
  userName: { fontWeight: 'bold', fontSize: 18, color: colors.DarkText },
  ...
  groupContainer: { backgroundColor: colors.SurfaceWhite, borderRadius: 16, borderWidth: 1, borderColor: colors.Slate200, overflow: 'hidden' },
  itemContainer: { flexDirection: 'row', alignItems: 'center', padding: 16, borderBottomWidth: 1, borderBottomColor: colors.Slate100 },
  itemLabel: { flex: 1, marginLeft: 16, fontSize: 16, color: colors.DarkText },
  creatorCTA: {
  ...
  logoutBtn: {
    flexDirection: 'row', width: '100%', padding: 16, backgroundColor: colors.SurfaceWhite,
    borderRadius: 12, borderWidth: 1, borderColor: colors.PrimaryRedAlpha10,
    alignItems: 'center', justifyContent: 'center',
  },
  modalOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center' },
  dialog: { backgroundColor: colors.SurfaceWhite, width: '85%', borderRadius: 16, padding: 24, elevation: 4 },
  radioOption: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12 },
  radioCircle: { height: 20, width: 20, borderRadius: 10, borderWidth: 2, borderColor: colors.Slate400, alignItems: 'center', justifyContent: 'center' },
  radioCircleActive: { borderColor: colors.PrimaryRed },
  radioDot: { height: 10, width: 10, borderRadius: 5, backgroundColor: colors.PrimaryRed },
});
